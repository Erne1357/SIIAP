...
(() => {
    ...
    async function loadEvents() {
        try {
            // Por ahora simulamos eventos - posteriormente conectar con tu API de eventos
            const mockEvents = [
                {
                    id: 1,
                    title: "Entrevistas Admisión 2025-1",
                    program_id: 1,
    ...
    async function handleAddWindow(e) {
        ...
        const payload = {
            day: document.getElementById('windowDate').value,
            start: document.getElementById('windowStartTime').value,
            end: document.getElementById('windowEndTime').value,
            slot_minutes: parseInt(document.getElementById('windowSlotMinutes').value, 10)
        };
        console.log('Adding window with payload:', payload);
        try {
    ...
    async function handleGenerateSlots() {
        ...
        try {
            // Esto requiere obtener las ventanas del evento y generar slots para cada una
            // Por simplicidad, asumimos que hay una ventana reciente
            const mockWindowId = 1; // En producción, obtener ventanas del evento

            const { data } = await apiRequest(`${API}/events/windows/${mockWindowId}/generate-slots`, {
                method: 'POST'
            });

            flash(`Se generaron ${data.created} nuevos horarios`, 'success');
            await loadSlots(selectedEventId);
